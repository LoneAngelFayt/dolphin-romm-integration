...
# Formats Dolphin can boot, best first: a folder holding several candidates
# picks by this order, so a .rvz beats the uncompressed .iso beside it and a
# real disc image beats a homebrew .dol dropped in the same folder.
# .m3u is deliberately absent, because Dolphin boots the disc itself and every
# folder shipping a playlist also ships the discs it points at.
ROM_EXTENSIONS = (
    ".rvz", ".iso", ".gcm", ".wbfs", ".wia", ".gcz", ".ciso", ".tgc",
    ".wad", ".dol", ".elf",
)
...
def _resolve_rom_file(path: Path) -> Path | None:
    ...
    `.../roms/gc/Metroid Prime` for a library laid out one game per folder.
...
class BrokerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # A folder-organized game arrives as its folder, which Dolphin cannot
